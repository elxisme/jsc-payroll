      ...
      <ScrollArea className="flex-1 px-4 lg:px-6 py-4">
        <nav className="space-y-2">
          {filteredNavigationGroups.map((group) => {
                ...
                <Link key={item.name} href={item.href} onClick={() => isMobile && setMobileMenuOpen(false)} asChild>
                  <Button
                    ...
                    className={cn(
                      ...
                      isActive ? 'bg-nigeria-green text-white shadow-sm hover:bg-nigeria-green' : 'text-gray-700 hover:text-gray-900 hover:bg-gray-50'
                    )}
                  >
                    <item.icon className={cn('mr-3 h-5 w-5 flex-shrink-0', isActive ? 'text-white' : 'text-gray-400 group-hover:text-gray-900')} />
                    {item.name}
                  </Button>
                </Link>
              );
            ...
            return (
              <Collapsible key={group.name} open={isGroupExpanded} onOpenChange={() => toggleGroup(groupKey)}>
                <CollapsibleTrigger asChild>
                  <Button
                    variant="ghost"
                    className={cn(
                      'group flex items-center justify-between w-full px-3 py-2 text-sm font-medium rounded-lg transition-colors',
                      groupIsActive ? 'bg-gray-100 text-gray-900 hover:bg-gray-100' : 'text-gray-700 hover:text-gray-900 hover:bg-gray-50'
                    )}
                  >
                    <div className="flex items-center">
                      <group.icon className={cn('mr-3 h-5 w-5 flex-shrink-0', groupIsActive ? 'text-nigeria-green' : 'text-gray-400 group-hover:text-gray-900')} />
                      <span className="font-medium">{group.name}</span>
                    </div>
                    {isGroupExpanded ? <ChevronDown className="h-4 w-4 text-gray-400" /> : <ChevronRight className="h-4 w-4 text-gray-400" />}
                  </Button>
                </CollapsibleTrigger>
                        ...
                        <Link key={item.name} href={item.href} onClick={() => isMobile && setMobileMenuOpen(false)} asChild>
                          <Button
                            ...
                            className={cn(
                              ...
                              isActive ? 'bg-nigeria-green text-white shadow-sm hover:bg-nigeria-green' : 'text-gray-600 hover:text-gray-900 hover:bg-gray-50'
                            )}
                          >
                            <item.icon className={cn('mr-3 h-4 w-4 flex-shrink-0', isActive ? 'text-white' : 'text-gray-400 group-hover:text-gray-600')} />
                            {item.name}
                          </Button>
                        </Link>
                      );
            ...
            <Link href="/notifications" onClick={() => isMobile && setMobileMenuOpen(false)} asChild>
              <Button
                ...
                className={cn(
                  ...
                  isCurrentPath('/notifications') ? 'bg-nigeria-green text-white shadow-sm hover:bg-nigeria-green' : 'text-gray-700 hover:text-gray-900 hover:bg-gray-50'
                )}
              >
                <Bell className={cn('mr-3 h-5 w-5 flex-shrink-0', isCurrentPath('/notifications') ? 'text-white' : 'text-gray-400 group-hover:text-gray-900')} />
                Notifications
        ...
        <div className="px-4 lg:px-6 py-4 border-t border-gray-200">
          <div className="flex items-center space-x-3 mb-3">
            <Avatar className="h-8 w-8">
              <AvatarFallback className="bg-nigeria-green text-white text-sm">{getUserInitials()}</AvatarFallback>
            </Avatar>
    ...
    <div className="h-screen flex overflow-hidden bg-gray-50">
      ...
      <div className="flex flex-col w-0 flex-1 lg:pl-80">
        <div className="sticky top-0 z-40 flex h-16 shrink-0 items-center gap-x-4 border-b border-gray-200 bg-white px-4 shadow-sm sm:gap-x-6 sm:px-6 lg:px-8">
          ...
          {/* CORRECTED: Mobile menu Sheet and Trigger are now together */}
              ...
              <UiTooltip>
                <TooltipTrigger asChild>
                  <Link href="/notifications" asChild>
                    <Button variant="ghost" size="sm" className={cn('relative transition-all duration-300', hasNewNotifications ? 'animate-pulse bg-blue-50 hover:bg-blue-100' : '')}>
                      <Bell className="h-5 w-5" />
                      {notificationCount && notificationCount > 0 && (
                        <span className={cn('absolute -top-1 -right-1 h-4 w-4 bg-red-500 text-white rounded-full text-xs flex items-center justify-center transition-all duration-300', hasNewNotifications ? 'animate-bounce scale-110' : '')}>
                          {notificationCount > 99 ? '99+' : notificationCount}
                        </span>
                      )}
                      {hasNewNotifications && <span className="absolute -top-1 -right-1 h-4 w-4 bg-blue-500 rounded-full animate-ping"></span>}
                    </Button>
                  </Link>
                </TooltipTrigger>
                <TooltipContent><p>View notifications ({notificationCount || 0} unread)</p></TooltipContent>
              </UiTooltip>
              <DropdownMenu>
                <UiTooltip>
                  <TooltipTrigger asChild>
                  ...
                  <TooltipContent><p>Account menu</p></TooltipContent>
                </UiTooltip>
          ...
          <footer className="mt-12 pt-8 border-t border-gray-200">
            <div className="text-center">
              <p className="text-gray-500" style={{ fontSize: '0.7em' }}>
                ...
                <a href="https://elxis.com.ng" target="_blank" rel="noopener noreferrer" className="text-nigeria-green hover:text-green-700 underline">
                  eLxis
                </a>
              </p>
            </div>
